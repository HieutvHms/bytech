  ...
  Future<FirmwareCheckResult?> checkCurrentFirmware() async {
    ...
    try {
      ...
      if (!connectivityResult.contains(ConnectivityResult.none)) {
        ...
        if (deviceMac.isNotEmpty && !deviceMac.contains('.')) {
          ...
          if (result != null) {
            ...
            if (!result.noUpdate) {
              // API báo có bản cập nhật
              isOnlineSuccess = true;
              showFirmwareUpdateDialog(
                  globalKey.currentContext!, this, result.updateUrl);
              return result; // Kết thúc
            } else {
              // API báo không có bản cập nhật mới (Hoặc API đang mock trả về noUpdate=true)
              // -> Ta KHÔNG return ở đây, cố tình để isOnlineSuccess = false
              // -> Để nó chạy tiếp xuống bước OFFLINE CHECK bên dưới (Xem có file trong máy không)
      ...
      if (!isOnlineSuccess) {
        ...
        */
        if (hardwareVersion != null) {
          // KHÔNG TÌM THẤY MAC, HOẶC API BÁO KHÔNG CÓ BẢN MỚI
          // -> Thử check xem trong máy có tải sẵn file cứu hộ không
